    ...
    if (ev.type == EV_SYN) {
        ...
        if (ev.code == REL_Y) {
            ...
            if (rel_sum > 70) {
                fake_key = 1;
                ev.type = EV_KEY;
                ev.code = VIRTUAL_KEY_MOUSE_DOWN;
                ev.value = 1;
                rel_sum = 0;
            } else if (rel_sum < -70) {
                fake_key = 1;
                ev.type = EV_KEY;
                ev.code = VIRTUAL_KEY_MOUSE_UP;
                ev.value = 1;
                rel_sum = 0;
            }
            ...
            if (ev.value > 0) {
                fake_key = 1;
                ev.type = EV_KEY;
                ev.code = VIRTUAL_KEY_MOUSE_WHEEL_UP;
                ev.value = 1;
            } else if (ev.value < 0) {
                fake_key = 1;
                ev.type = EV_KEY;
                ev.code = VIRTUAL_KEY_MOUSE_WHEEL_DOWN;
                ev.value = 1;
            }
        }
    } else if ((ev.type == EV_ABS) && (middley > 0)) {
        if (ev.code == ABS_Y || ev.code == ABS_MT_POSITION_Y) {
            //FIXME
            if( time(NULL) - time_touch > 1 ){
                time_touch =  time(NULL);
                if (ev.value > middley) {
                    fake_key = 1;
                    ev.type = EV_KEY;
                    ev.code = VIRTUAL_KEY_TOUCH_DOWN;
                    ev.value = 1;
                } else if (ev.value < middley) {
                    fake_key = 1;
                    ev.type = EV_KEY;
                    ev.code = VIRTUAL_KEY_TOUCH_UP;
                    ev.value = 1;
                }
            }
        }
...
int ui_menu_select(int sel) {
    ...
    if (show_menu > 0) {
        old_sel = menu_sel;
        menu_sel = sel;
#ifdef RECOVERY_MENU_LOOP_SELECT
        if (menu_sel < 0) menu_sel = menu_items - 1;
        if (menu_sel >= menu_items) menu_sel = 0;
#else
        if (menu_sel < 0) menu_sel = 0;
        if (menu_sel >= menu_items) menu_sel = menu_items-1;
#endif /* RECOVERY_VOLUMEDOWN_SELECT */
        sel = menu_sel;
